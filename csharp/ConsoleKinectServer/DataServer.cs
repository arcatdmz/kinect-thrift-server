...
namespace ConsoleKinectServer
{
    class DataServer
    {
        private KinectServiceHandler kinect;
        ...
        public DataServer(KinectServiceHandler kinect, int numConnections, int receiveBufferSize)
        {
            this.kinect = kinect;
            m_totalBytesRead = 0;
        ...
        private void ProcessSend(SocketAsyncEventArgs e)
        {
            ...
            {
                Console.WriteLine("{0} bytes transferred.", e.BytesTransferred);

                // done echoing data back to the client
                AsyncUserToken token = (AsyncUserToken)e.UserToken;
                token.Sent += e.BytesTransferred;
                if (token.Sent < token.Sending)
                {
                    Console.WriteLine("{0} bytes sent. Status incomplete.", token.Sent);
                    byte[] data = ((AsyncUserToken)e.UserToken).Data;
                    e.SetBuffer(data, token.Sent, token.Sending - token.Sent);
                    bool willRaiseEvent = token.Socket.SendAsync(e);
                    if (!willRaiseEvent)
                    {
                        ProcessSend(e);
